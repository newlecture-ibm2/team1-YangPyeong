...
/**
 * 기상청 일일 날씨 기록 비즈니스 로직 처리 서비스.
 * 스케줄러 계층과 트랜잭션 계층을 분리하기 위해 독립된 서비스로 구성합니다.
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class DailyWeatherRecordService {
    ...
    @Transactional
    public void doRecordDailyWeather() {
        LocalDate yesterday = LocalDate.now().minusDays(1);
        log.info("[DailyWeatherRecordService] 전날({}) 날씨 기록 배치를 시작합니다.", yesterday);
