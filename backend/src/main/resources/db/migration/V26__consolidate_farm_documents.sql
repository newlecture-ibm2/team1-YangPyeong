-- V26__consolidate_farm_documents.sql
...
-- 2. 기존 데이터 이관 (document_url, land_cert_image_url → documents JSONB)
UPDATE farms SET documents = (
  COALESCE(
    (SELECT jsonb_agg(doc) FROM (
      ...
      UNION ALL
      SELECT jsonb_build_object('type', 'OTHER', 'url', document_url, 'name', 'document') AS doc
...
-- 3. 레거시 컬럼 삭제 (5개)
ALTER TABLE farms DROP COLUMN IF EXISTS document_url;
