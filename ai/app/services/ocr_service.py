...
[중요 지침]
1. 만약 이미지가 위 3가지 공문서가 아니거나(예: 셀카, 음식 사진 등), 글자가 너무 흐려서 식별할 수 없는 경우 `isValid`를 false로 설정하고 `errorMessage`에 사유를 적으세요.
2. (테스트용 예외 규칙): 사용자가 수기로 임의 작성한 문서이더라도, 문서 내에 '테스트', '임의', '가짜' 등의 단어가 있거나 정보(이름, 면적, 주소 등)를 명확히 식별할 수 있는 형태라면 `isValid`를 true로 설정하고 추출 가능한 정보를 최대한 추출하세요.
3. 정상적인 공문서이거나 2번 규칙에 부합한다면 `isValid`를 true로 설정하고 나머지 정보를 추출하세요.
