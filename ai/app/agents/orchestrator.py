...
- guidance_agent: 작물 추천·재배·농장 등록, AI 추천·수익 화면 **이동 안내** (데이터 조회 X)
- recommend_agent: **최신 AI 추천 결과** — 순위, 점수, TOP3, 특정 작물 적합도, 코칭 상태 조회·해석
...
"작물 추천해줘" / "뭐 키울까" → guidance_agent (화면 안내)
"1등 작물 뭐야" / "추천 순위" / "감자 몇 점" / "TOP3" → recommend_agent
...
"안녕하세요!" → general_chat
"커뮤니티에 배추 재배 팁 있어?" → community_agent
"내 농장 흙 상태에 맞는 작물 추천해줘" → farm_agent, balance_agent
"양평군 쌀 수급 현황이랑 관련 지원 정책 알려줘" → gov_agent, policy_agent
"감자 심으려는데 보조금이랑 시세 전망 같이 알려줘" → farm_agent, policy_agent, balance_agent
"내 농장에 맞는 작물 등록하고 싶어" → farm_agent, balance_agent

## 추천 + 다른 도메인 복합 질문 예시
"감자랑 배추 비교해줘" → recommend_agent
"토양 점수 높은 순으로 보여줘" → recommend_agent
"추천 1등 작물 시세 알려줘" → recommend_agent, balance_agent
"나한테 맞는 작물 뭐야?" → recommend_agent
"다음 시즌에 뭐 심을까?" → recommend_agent
"분석 언제 했어?" → recommend_agent
"추천 작물 중 보조금 있는 거" → recommend_agent, policy_agent
"추천 작물 장터에 팔고 싶어" → recommend_agent, shop_agent
"추천 작물 재배법 알려줘" → recommend_agent, farm_agent"""
