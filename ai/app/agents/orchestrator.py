...
- farm_agent: 내 농장 상태, 면적, 재배 이력, 날씨, 토양, 병해충, 기상 재해 예방 및 대응법, 내 농장 관리
...
- recommend_agent: **AI 작물 추천 및 공식 재배 정보** — 추천 작물, 순위, 비교, 분석 이력, 특정 작물의 수확 시기, 재배법, 파종 시기, 품종 추천
- general_chat: 단순 인사, 팜밸런스 서비스 안내, 특정 카테고리에 속하지 않는 모든 일반 질문 및 기타 지식 검색

## 판단 예시
"내 농장 면적 알려줘" → farm_agent
"처음인데 어떻게 시작해?" / "나 이제 뭐해?" / "추천 화면으로 가줘" → guidance_agent (온보딩/이동)
"작물 추천해줘" / "나 초보인데 뭐 키울까" / "수확 시기 언제야" / "콩 재배법 알려줘" / "품종 추천" → recommend_agent
...
DOMAIN_FORCE_KEYWORDS: dict[str, set[str]] = {
  # account · guidance — shop보다 먼저 매칭 (구체 키워드 우선)
    ...
    "recommend_agent": {
        ...
        "상위 작물", "하위 작물", "꼴등",
        # 신규 추가 - 비교
        "추천 작물 비교", "작물 비교", "top3 비교", "top 3 비교",
        ...
        "코칭해줘", "코칭 해줘", "맞춤 작물",
        # 농작업 일정 및 재배법 추가
        "수확 시기", "언제 수확", "수확 언제", "수확", "파종", "재배법", "재배 일정",
        "품종", "특징이 뭐야", "특성 알려줘",
    },
    "shop_agent": {
        # 장바구니
        ...
        "상품 상세설명", "상품 상세 설명", "상품 상세화면", "상품 상세 화면", "상품 상세페이지", "상품 상세 페이지",
        # 판매자 상품 수정/삭제/상태변경
    ...
    "farm_agent": {
        "내 농장", "토양", "흙", "면적", "재배 이력", "날씨", "기후", "병해충", "내 농장 관리",
        "가뭄", "폭염", "장마", "대비 어떻게", "관리법", "기상 재해",
        "농장 면적", "가용면적", "물 주기",
...
async def call_farm_agent(state: AgentState):
    ...
    try:
        ...
        agent = get_farm_agent(farm_id, state.get("analysis_context"))
...
2. 성공한 분석이 하나도 없으면, "현재 해당 조건의 정보를 확인하지 못했습니다. 질문을 더 구체적으로(예: 작물명, 지역명 포함) 해주시면 정확한 분석을 도와드리겠습니다."라고 안내하세요.
3. 여러 도메인의 결과가 있을 때는 아래 구조로 답변을 작성하세요:
   - **첫 줄(핵심 요약)**: 가장 중요한 결론을 한두 문장으로 요약
   - **상세 분석**: 불릿(•)이나 번호 목록으로 나눠서 간결하게 정리

## 📐 답변 형식 (반드시 지키세요)
1. **분량**: 기본적으로 **짧고 핵심만**(6~8줄) 전달하세요. 복수 분석 결과를 종합해야 할 때는 늘려도 되지만, 반드시 불릿·번호로 구조화하세요. 주절거리지 마세요.
2. **구조화 필수**: 긴 설명을 한 덩어리로 쓰지 마세요. 줄바꿈·불릿(•)·번호로 나누세요.
3. **핵심 앞에 배치**: 가장 중요한 결론(작물명·순위·점수·수치)을 첫 줄에 넣으세요.
4. **용어 치환**: 아래 규칙에 따라 개발/내부 용어를 반드시 바꿔 쓰세요.
   - DB, 데이터베이스 → '저장된 정보' 또는 '기록'
   - 농사로 → '공식 재배 정보'
   - API → 언급하지 않기 (배경 설명 불필요)
   - AI 코칭, AI 분석 → '맞춤형 재배 조언' 또는 '전문 분석'
   - 시스템, 서버, 프로그램, 에이전트, 모듈, 파이프라인 → 절대 언급 금지
   - 그 외 개발 용어가 떠오르면, 농업인이 이해할 수 있는 일상어로 바꿔 쓰세요.
5. "에이전트가 말하길...", "분석팀 보고에 따르면..." 같은 내부 과정을 절대 노출하지 마세요. 직접 분석한 것처럼 자연스럽게 전달하세요.

## 말투
- 100% 한국어(한글)로만 답변. 영어·한자·외국어 절대 금지.
- 전문적이면서도 따뜻한 존댓말. 이모지는 핵심 포인트에만 1~2개.
...
async def synthesizer_node(state: AgentState):
    ...
    try:
        ...
        # 에이전트 답변 수집 (이번 턴에 호출된 에이전트 개수만큼만 뒤에서부터 추출)
        is_multi = len(routed) >= 2
        num_agents = len(routed) if routed else 1
        
        all_ai_msgs_history = [msg for msg in state["messages"] if isinstance(msg, AIMessage)]
        # 전체 히스토리 중 가장 최근의 num_agents 개수만 사용
        all_ai_msgs = all_ai_msgs_history[-num_agents:] if all_ai_msgs_history else []
        
        for msg in all_ai_msgs:
            logger.info(f"[Synthesizer Debug] Agent raw response: {msg.content[:200]}")
        ...
        # 사용자의 원본 질문 추출 (마지막 HumanMessage)
        user_question = next(
            ...
            try:
                ...
                async with _httpx.AsyncClient(timeout=3.0) as client:
                    ...
                    if res.status_code == 200:
                        ...
                        # 토양 정보 추출
                        soil_type = status.get("soilType")
                        ph = status.get("ph")
                        om = status.get("organicMatter")
                        if soil_type or ph or om:
                            soil_info = []
                            if soil_type: soil_info.append(f"토성: {soil_type}")
                            if ph: soil_info.append(f"pH: {ph}")
                            if om: soil_info.append(f"유기물: {om}%")
                            analysis_context["soil_info"] = ", ".join(soil_info)
                        
                        logger.info("[ContextResolver] farm_id=%s → crops=%s, soil=%s", farm_id, active_crops, analysis_context.get("soil_info"))
            except Exception as e:
