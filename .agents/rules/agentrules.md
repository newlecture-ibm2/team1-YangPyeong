...
- **커밋(Commit) 및 푸시(Push) 권한**: 사용자가 명시적으로 요청하기 전까지는 **절대 에이전트가 임의로 로컬 브랜치에 커밋(Commit)하거나 원격 저장소에 푸시(Push)하지 않습니다.** (이전 대화(세션)에서 사용자가 허락했었더라도, 새로운 대화에서는 권한이 초기화되므로 반드시 매번 새로 허락을 받아야 합니다.)
