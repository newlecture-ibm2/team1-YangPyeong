...
export default function CropGuide({ rec, recommendResult }: CropGuideProps) {
  ...
  const openDetailedGuide = useCallback(async () => {
    ...
    try {
      ...
      const cached = await getCachedCropDetailedGuide(farmId, rec.cropId, experienceLevel);

      if (cached?.topics?.length) {

        setDisplayGuide(

          mergeGuideWithStructuredPests(mapServerGuideToDetailedGuide(cached), rec),
      ...
      setDisplayGuide(
        ...
        mergeGuideWithStructuredPests(mapServerGuideToDetailedGuide(generated), rec),
